...
void auto_ref_strip() {
    int v = 4;
    int& vRef = v;
    [[maybe_unused]] auto vCopy = vRef; // auto will strip the reference
...
void decltype_auto() {
    decltype(auto) v = auto_work::utils::val_ref_wrapper();
    decltype(auto) vRefDcl = auto_work::utils::decltype_val_ref_wrapper();
    ...
    decltype(auto) vUpdate = auto_work::utils::val_ref_wrapper();
    decltype(auto) vRefDclUpdate = auto_work::utils::decltype_val_ref_wrapper();
